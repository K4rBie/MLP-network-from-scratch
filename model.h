...
public:
    ...
    void train(const path &training_data_path, int epochs, float learning_rate);
    void evaluate(const path &eval_data_path);
    Matrix infer(Matrix image);
    

private:
    bool classesFileExists(const path &image_path);
    Matrix fetchLabels(const path& image_path);
